...
function Install-OrcishBridge([string]$GameDir) {
    ...
    if ($modsText -notmatch '(?im)^\s*OrcishScout\s*:\s*1\s*
    ...
    $runtimePresent = (Test-Path (Join-Path $gameDir "UE4SS.dll")) -or (Test-Path (Join-Path $gameDir "ue4ss\UE4SS.dll")) -or (Test-Path (Join-Path $gameDir "dwmapi.dll"))

    if ($isWindowsApps -and -not $AllowWindowsAppsInstall) {
        Write-Warn "Detected Microsoft Store/Xbox WinGDK build under WindowsApps."
        if ($runtimePresent) {
            Write-Host "UE4SS runtime already exists; refreshing only the OrcishScout Lua bridge..."
            try {
                $bridgeResult = Install-OrcishBridge $gameDir
                Write-Ok "OrcishScout bridge refreshed: $($bridgeResult.BridgePath)"
                Write-Ok "Bridge output: $($bridgeResult.OutputPath)"
            } catch {
                Write-Warn "Bridge refresh could not write to WindowsApps: $($_.Exception.Message)"
                Write-Warn "Run PowerShell as Administrator and rerun with -AllowWindowsAppsInstall."
            }
        } else {
            Write-Warn "UE4SS runtime is not installed. Full WindowsApps installation remains disabled by default."
            Write-Warn "Run PowerShell as Administrator and rerun with -AllowWindowsAppsInstall."
        }
        return
    }
    ...
    Write-Ok "OrcishScout bridge: $($bridgeResult.BridgePath)"
    Write-Ok "Bridge output: $($bridgeResult.OutputPath)"
}
...
function Show-Status {
    ...
    if ($exe) {
        ...
        if ($bridgeInfo) {
            ...
            if (Test-Path $log) {
                $hits = Select-String -Path $log -Pattern "OrcishScout|bridge loading|bridge ready|Cannot open telemetry" -SimpleMatch:$false -ErrorAction SilentlyContinue | Select-Object -Last 12
                if ($hits) {
                    Write-Host "  Recent OrcishScout UE4SS log lines:"
                    $hits | ForEach-Object { Write-Host ("    " + $_.Line) }
                } else { Write-Warn "UE4SS.log contains no OrcishScout bridge messages. The Lua mod may not have loaded yet." }
...
Write-Ok "Toolkit pass complete."
) { Add-Content $modsFile "OrcishScout : 1" }
    ...
    return [pscustomobject]@{ BridgePath=$bridgeDst; OutputPath=$outFile }
}
